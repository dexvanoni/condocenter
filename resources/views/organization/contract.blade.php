...
<div class="container-fluid px-4">
    <div class="mb-4 d-flex flex-wrap justify-content-between gap-3">
        <div>
            <a href="{{ route('organization.dashboard') }}" class="text-decoration-none"><i class="bi bi-arrow-left"></i> {{ $organization->displayName() }}</a>
            <h1 class="mt-2 mb-1"><i class="bi bi-receipt-cutoff"></i> Contrato SindCON</h1>
            <p class="text-muted mb-0">Acompanhe a vigência, os valores e as cobranças geradas no Asaas.</p>
        </div>
    </div>

    @if($contracts->isEmpty())
        <div class="alert alert-info">Nenhum contrato foi vinculado a esta administradora ainda. A plataforma precisa cadastrar o plano.</div>
    @else
                        ...
                        <dl class="row mb-0">
                            ...
                            <dd class="col-sm-7">
                                ...
                                @if($subscription->auto_renew)
                                    <span class="badge bg-info text-dark">Autorrenova</span>
                                @endif
                            </dd>
                            <dt class="col-sm-5">Condomínios</dt>
                            <dd class="col-sm-7">{{ $subscription->max_condominiums ?? 'Sem limite' }}</dd>
                            <dt class="col-sm-5">Unidades</dt>
